...
import { del, save, getOrgList, findVisitStatistics } from './service';

const { dvaModel } = utils;
const { modelExtend, model } = dvaModel;

export default modelExtend(model, {
  namespace: 'pmVisitStatistics',
  ...
  effects: {
    ...
    *findVisitStatistics({ payload }, { call }) {
      const result = yield call(findVisitStatistics, payload);
      const { success, message: msg } = result || {};
      message.destroy();
      if (success) {
        // message.success(msg);
